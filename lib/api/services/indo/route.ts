...
import { IndoServiceHandler } from "@/lib/services/indo.service";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const service = ServiceRegistry.get(
      "indo",
      request,
    ) as unknown as IndoServiceHandler;
    ...
    const result = await service.listInpatients(token, filters, {
      page,
      limit,
    });
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const updatedAdmission = await service.addDailyProgress(
      token,
      admissionId,
      progress,
    );
