...
export async function GET(request: NextRequest) {
  try {
    ...
    const service = ServiceRegistry.get('ot', request);
    ...
    if (surgeryDate) filters['schedule.surgeryDate'] = { $gte: new Date(surgeryDate) };
    if (surgeonId) filters['team.surgeon'] = surgeonId;
    
    const result = await service.listOTSchedules(
      token,
      filters,
      { page, limit }
    );
...
export async function POST(request: NextRequest) {
  try {
    ...
    const otSchedule = await service.createOTSchedule(token, {
      ...data,
      otId,
      status: 'scheduled',
      preOpChecklist: {
