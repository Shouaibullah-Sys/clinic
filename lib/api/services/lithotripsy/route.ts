...
import { LithotripsyServiceHandler } from "@/lib/services/lithotripsy.service";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const service = ServiceRegistry.get(
      "lithotripsy",
      request,
    ) as unknown as LithotripsyServiceHandler;
    ...
    const result = await service.listLithotripsyProcedures(token, filters, {
      page,
      limit,
    });
