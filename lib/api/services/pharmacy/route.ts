...
export async function GET(request: NextRequest) {
  try {
    ...
    const service = ServiceRegistry.get('pharmacy', request);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const prescription = await service.createPrescription(token, {
      ...data,
      prescriptionId,
      status: 'pending',
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const user = await authorizeServiceAccess(request, 'pharmacy', 'administer');
    const service = ServiceRegistry.get('pharmacy', request);
    const { prescriptionId, items } = await request.json();
    
    const prescription = await service.dispenseMedication(token, prescriptionId, items);
