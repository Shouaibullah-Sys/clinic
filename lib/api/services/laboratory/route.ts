...
export async function GET(request: NextRequest) {
  try {
    ...
    const service = ServiceRegistry.get('laboratory', request);
    ...
    const result = await service.listLabTests(
      token,
      filters,
      { page, limit }
    );
...
export async function POST(request: NextRequest) {
  try {
    ...
    const labTest = await service.createLabTest(token, {
      ...data,
      labId,
      status: 'ordered',
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const labTest = await service.submitTestResults(token, testId, results);
