...
import { AmbulanceServiceHandler } from "@/lib/services/ambulance.service";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const service = ServiceRegistry.get<AmbulanceServiceHandler>(
      "ambulance",
      request,
    );
    ...
    const result = await service.listAmbulanceTrips(token, filters, {
      page,
      limit,
    });
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const ambulanceTrip = await service.updateTripStatus(
      token,
      tripId,
      status,
      location,
