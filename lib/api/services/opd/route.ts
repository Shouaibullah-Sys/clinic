...
import { OpdServiceHandler } from "@/lib/services/opd.service";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const service = ServiceRegistry.get(
      "opd",
      request,
    ) as unknown as OpdServiceHandler;
    ...
    const result = await service.listOpdVisits(token, filters, { page, limit });
