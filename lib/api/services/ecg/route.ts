...
export async function GET(request: NextRequest) {
  try {
    ...
    const service = ServiceRegistry.get('ecg', request);
    ...
    const result = await service.listECGRecords(
      token,
      filters,
      { page, limit }
    );
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const ecgRecord = await service.submitInterpretation(token, ecgId, interpretation);
