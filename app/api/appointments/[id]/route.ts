// app/api/appointments/[id]/route.ts - UPDATED
...
export async function GET(
  ...
  try {
    ...
    console.log(`Fetching appointment ${appointmentId} for user ${userId} (${userRole})`);
    
    // Get appointment with basic info
    const appointment = await Appointment.findById(appointmentId)
      .populate("patient", "name phone email patientId dateOfBirth gender address bloodGroup")
      .populate("doctor", "name specialization department licenseNumber phone email")
    ...
    // Check permissions
    const isDoctorOwner = userRole === "doctor" && appointment.doctor?._id?.toString() === userId;
    ...
    const labTests = await LabTest.find({ 
      ...
      .select("testId testName category price discountedPrice status priority specimen charges orderedAt collectedAt completedAt reportedAt notes")
      .sort({ orderedAt: -1 })
      .lean();
    
    console.log(`Found ${labTests.length} lab tests for appointment ${appointmentId}`);
    ...
    const prescriptionsTotal = prescriptions.reduce((sum, prescription) => {
      if (prescription.medications && Array.isArray(prescription.medications)) {
        return sum + prescription.medications.reduce((medSum: number, med: any) => 
          medSum + (med.quantity || 1) * (med.price || 0), 0);
      }
      return sum;
    }, 0);
    ...
    const paidAmount = labTests.reduce((sum, test) => 
      sum + (test.charges?.paid || 0), 0);
    
    const dueAmount = labTests.reduce((sum, test) => 
      sum + (test.charges?.due || 0), 0);
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        billingSummary: {
          ...
          paidAmount,
          dueAmount,
          paymentStatus: dueAmount === 0 ? "fully_paid" : paidAmount > 0 ? "partially_paid" : "pending"
        }
      },
    });
