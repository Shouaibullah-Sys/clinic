    ...
    try {
      ...
      if (data.success) {
        ...
        if (data.data.results?.parameters?.length > 0) {
          setParameters(
            data.data.results.parameters.map((p: any, index: number) => ({
              id: (index + 1).toString(),
              name: p.name || "",
              value: p.value || "",
              unit: p.unit || "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = {
        ...
        status: "pending", // Keep status as pending until sample is collected
        processingStatus: "completed", // Mark processing as completed
      };

      const response = await fetch(
        `/api/laboratory/tests/${params.id}/results`,
        {
          method: "PUT",
          headers: {
            Authorization: `Bearer ${accessToken}`,
      ...
      if (response.ok) {
        alert(
          "Test parameters added successfully! Now ready to collect sample.",
        );
